...
export function ArtisanEcommerceTemplate({ siteData, subdomain }: ArtisanEcommerceTemplateProps) {
  ...
  const testimonialsToDisplay = siteData.testimonials && siteData.testimonials.length > 0
    ...
    : [
        { quote: "J'ai commandé Miira-Cell+ et je suis absolument ravi du résultat. Le distributeur a su comprendre exactement ce que je voulais et le produit final est encore plus efficace que ce que j'imaginais.", author: "Marie Diop", location: siteData.businessLocation || "Dakar", avatar: "https://randomuser.me/api/portraits/women/32.jpg" },
        { quote: "Service de livraison rapide et efficace pour mes produits Revoobit. Le prix était raisonnable et le travail soigné. Je recommande vivement cette boutique pour la qualité de ses services.", author: "Jean Ndiaye", location: siteData.businessLocation || "Pikine", avatar: "https://randomuser.me/api/portraits/men/54.jpg" },
        { quote: "La personnalisation du pack Revoobit que j'ai offert en cadeau était parfaite. La présentation était précise et le produit de grande qualité. Livraison rapide et emballage soigné.", author: "Fatou Sarr", location: siteData.businessLocation || "Guédiawaye", avatar: "https://randomuser.me/api/portraits/women/67.jpg" },
      ];

  const skillsToDisplay = siteData.skills && siteData.skills.length > 0
    ? siteData.skills
    : [
        { icon: "PencilRuler", title: "Conseil Personnalisé Revoobit", description: "Création de programmes de bien-être uniques selon vos besoins." },
        { icon: "Wrench", title: "Suivi Client Revoobit", description: "Accompagnement et suivi pour une utilisation optimale des produits." },
        { icon: "Palette", title: "Présentation Produit Revoobit", description: "Des démonstrations claires et complètes des produits Revoobit." },
      ];
  ...
  return (
    <div className="font-sans antialiased text-gray-800 bg-white overflow-x-hidden">
          ...
          <nav className="flex justify-between items-center py-4">
            <div className="flex items-center gap-2"> {/* Adjusted gap for mobile */}
              ...
              <div className="flex flex-col">
                ...
                <p className={cn("text-xs font-medium", primaryColorTextClass)}>Distributeur Revoobit Officiel</p> {/* Adjusted text size for mobile */}
              </div>
            </div>
            <div className="hidden md:flex items-center gap-6"> {/* Adjusted gap for mobile */}
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-lg py-4 z-40">
            <nav className="flex flex-col items-center gap-4">
              ...
              {sectionsVisibility.showProductsServices && products.length > 0 && <a href="#produits" onClick={(e) => handleSmoothScroll(e, '#produits')} className="text-gray-700 font-medium hover:text-blue-600 transition-colors w-full text-center py-2 text-base">Produits Revoobit</a>} {/* Adjusted text size for mobile */}
              ...
              {sectionsVisibility.showTestimonials && testimonialsToDisplay.length > 0 && (
                <a href="#testimonials" onClick={(e) => handleSmoothScroll(e, '#testimonials')} className="text-gray-700 font-medium hover:text-blue-600 transition-colors w-full text-center py-2 text-base">Avis Clients Revoobit</a>
              )}
      ...
      {sectionsVisibility.showHero && (
        <section id="accueil" className={cn("relative py-16 md:py-24 text-white text-center bg-cover bg-center px-4", secondaryColorClass)} style={{ backgroundImage: siteData.heroBackgroundImage ? `linear-gradient(135deg, rgba(44, 62, 80, 0.9) 0%, rgba(231, 76, 60, 0.8) 100%), url('${siteData.heroBackgroundImage}')` : undefined }}> {/* Adjusted padding for mobile */}
          <div className="container mx-auto max-w-4xl"> {/* Removed px-4 md:px-6, using container mx-auto */}
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold mb-4 leading-tight">{siteData.heroSlogan || "Votre boutique Revoobit en ligne pour une vie saine !"}</h2> {/* Adjusted text size for mobile */}
            <p className="text-base md:text-xl mb-8 opacity-90">{siteData.aboutStory || "Découvrez les produits Miira-Cell+ et Miiralife pour votre bien-être."}</p> {/* Adjusted text size for mobile */}
            <div className="flex flex-col sm:flex-row gap-4 justify-center"> {/* Adjusted gap for mobile */}
              <a href="#produits" onClick={(e) => handleSmoothScroll(e, '#produits')} className={cn("inline-flex items-center gap-3 px-6 py-3 rounded-lg font-bold text-base bg-white text-red-600 hover:bg-gray-100 transition-all duration-300 ease-in-out transform hover:-translate-y-1 shadow-lg w-full sm:w-auto", primaryColorTextClass)}> {/* Adjusted padding, text size, and width for mobile */}
                <ShoppingCart className="h-5 w-5" /> Voir les produits Revoobit
              </a>
              <a href="#services" onClick={(e) => handleSmoothScroll(e, '#services')} className="inline-flex items-center gap-3 px-6 py-3 rounded-lg font-bold text-base bg-transparent border-2 border-white text-white hover:bg-white hover:text-red-600 transition-all duration-300 ease-in-out transform hover:-translate-y-1 shadow-lg w-full sm:w-auto"> {/* Adjusted padding, text size, and width for mobile */}
                <Wrench className="h-5 w-5" /> Découvrir les services Revoobit
              </a>
            </div>
          </div>
        </section>
        ...
        <div className="container mx-auto max-w-5xl"> {/* Removed px-4 md:px-6, using container mx-auto */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6"> {/* Adjusted grid and gap for mobile */}
            <div className="bg-white rounded-lg p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2"> {/* Adjusted padding for mobile */}
              ...
              <p className="text-gray-600 text-sm">Expédition sous 48h pour tous les produits Revoobit en stock</p> {/* Adjusted text size for mobile */}
            </div>
            <div className="bg-white rounded-lg p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2"> {/* Adjusted padding for mobile */}
              <div className={cn("h-16 w-16 rounded-full flex items-center justify-center text-white text-2xl mx-auto mb-4", primaryColorClass)} style={{ background: `linear-gradient(135deg, var(--${siteData.primaryColor}-600), var(--${siteData.secondaryColor}-500))` }}> {/* Adjusted size for mobile */}
                <ShieldCheck className="h-8 w-8" /> {/* Adjusted size for mobile */}
              </div>
              <h3 className={cn("text-lg font-semibold mb-2", secondaryColorTextClass)}>Garantie Qualité Revoobit</h3> {/* Adjusted text size for mobile */}
              <p className="text-gray-600 text-sm">Tous nos produits Revoobit sont garantis 12 mois</p> {/* Adjusted text size for mobile */}
            </div>
            <div className="bg-white rounded-lg p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2"> {/* Adjusted padding for mobile */}
              <div className={cn("h-16 w-16 rounded-full flex items-center justify-center text-white text-2xl mx-auto mb-4", primaryColorClass)} style={{ background: `linear-gradient(135deg, var(--${siteData.primaryColor}-600), var(--${siteData.secondaryColor}-500))` }}> {/* Adjusted size for mobile */}
            ...
            <div className="bg-white rounded-lg p-6 text-center shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2"> {/* Adjusted padding for mobile */}
              ...
              <h3 className={cn("text-lg font-semibold mb-2", secondaryColorTextClass)}>Support Client Revoobit</h3> {/* Adjusted text size for mobile */}
        ...
        <section id="produits" className="py-12 bg-white px-4"> {/* Adjusted padding for mobile */}
          <div className="container mx-auto max-w-5xl"> {/* Removed px-4 md:px-6, using container mx-auto */}
            <div className="text-center mb-8 md:mb-12"> {/* Adjusted mb for mobile */}
              ...
              <p className="mt-6 text-base text-gray-600 max-w-2xl mx-auto">Découvrez une sélection de nos produits Revoobit les plus populaires</p> {/* Adjusted mt and text size for mobile */}
            </div>
            ...
            <div className="flex justify-center gap-2 mb-8 flex-wrap"> {/* Adjusted gap and flex-wrap for mobile */}
              ...
              <button className={cn("px-4 py-1.5 rounded-full font-semibold transition-all duration-300 border text-sm", primaryColorBorderClass, secondaryColorTextClass, secondaryColorHoverBgClass, "hover:text-white")}>Miira-Cell+</button> {/* Adjusted padding and text size for mobile */}
              <button className={cn("px-4 py-1.5 rounded-full font-semibold transition-all duration-300 border text-sm", primaryColorBorderClass, secondaryColorTextClass, secondaryColorHoverBgClass, "hover:text-white")}>Miiralife</button> {/* Adjusted padding and text size for mobile */}
              <button className={cn("px-4 py-1.5 rounded-full font-semibold transition-all duration-300 border text-sm", primaryColorBorderClass, secondaryColorTextClass, secondaryColorHoverBgClass, "hover:text-white")}>Autres</button> {/* Adjusted padding and text size for mobile */}
            </div>
              ...
              {products.map((product: typeof siteData.productsAndServices[number], index: number) => (
                <div key={index} className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 relative group">
                  ...
                  <div className="p-4"> {/* Adjusted padding for mobile */}
                    <span className={cn("text-xs font-medium mb-1 block", accentColorTextClass)}>Produits Revoobit</span> {/* Placeholder category, adjusted text size for mobile */}
      ...
      {sectionsVisibility.showProductsServices && services.length > 0 && (
        <section id="services" className="py-12 bg-gray-100 px-4"> {/* Adjusted padding for mobile */}
          <div className="container mx-auto max-w-5xl"> {/* Removed px-4 md:px-6, using container mx-auto */}
            <div className="text-center mb-8 md:mb-12"> {/* Adjusted mb for mobile */}
              <h2 className={cn("text-2xl md:text-4xl font-bold mb-4 relative inline-block", secondaryColorTextClass)}> {/* Adjusted text size for mobile */}
                Nos Services Revoobit Sur Mesure
                <span className={cn("absolute bottom-[-15px] left-1/2 -translate-x-1/2 w-20 h-1 rounded-lg", primaryColorClass)}></span> {/* Adjusted width for mobile */}
              </h2>
              <p className="mt-6 text-base text-gray-600 max-w-2xl mx-auto">Des prestations adaptées à vos besoins spécifiques avec Revoobit</p> {/* Adjusted mt and text size for mobile */}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"> {/* Adjusted grid and gap for mobile */}
              {services.map((service: typeof siteData.productsAndServices[number], index: number) => (
                <div key={index} className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 border-t-4" style={{ borderColor: `var(--${siteData.secondaryColor}-500)` }}> {/* Adjusted padding for mobile */}
                  ...
                  <ul className="list-none space-y-2 mb-6 text-left text-sm"> {/* Adjusted space-y and text size for mobile */}
                    <li className="flex items-center gap-2"><Check className={cn("h-4 w-4", successColorTextClass)} /> Consultation personnalisée Revoobit</li> {/* Adjusted size for mobile */}
                    <li className="flex items-center gap-2"><Check className={cn("h-4 w-4", successColorTextClass)} /> Suivi de votre bien-être</li> {/* Adjusted size for mobile */}
                    <li className="flex items-center gap-2"><Check className={cn("h-4 w-4", successColorTextClass)} /> Accompagnement pour votre réseau</li> {/* Adjusted size for mobile */}
                    <li className="flex items-center gap-2"><Check className={cn("h-4 w-4", successColorTextClass)} /> Conseils produits Revoobit</li> {/* Adjusted size for mobile */}
                  </ul>
                  <a href={`https://wa.me/${siteData.whatsappNumber}?text=Je%20suis%20intéressé%20par%20le%20service%20${service.title}%20Revoobit`} target="_blank" rel="noopener noreferrer" className={cn("w-full py-2.5 rounded-lg font-bold text-white text-sm transition-colors duration-300 flex items-center justify-center gap-2", secondaryColorClass, secondaryColorHoverBgClass)}> {/* Adjusted padding and text size for mobile */}
      ...
      {sectionsVisibility.showTestimonials && testimonialsToDisplay.length > 0 && (
        <section className="py-12 bg-white px-4"> {/* Adjusted padding for mobile */}
          <div className="container mx-auto max-w-5xl"> {/* Removed px-4 md:px-6, using container mx-auto */}
            <div className="text-center mb-8 md:mb-12"> {/* Adjusted mb for mobile */}
              <h2 className={cn("text-2xl md:text-4xl font-bold mb-4 relative inline-block", secondaryColorTextClass)}> {/* Adjusted text size for mobile */}
                Avis Clients Revoobit
                <span className={cn("absolute bottom-[-15px] left-1/2 -translate-x-1/2 w-20 h-1 rounded-lg", primaryColorClass)}></span> {/* Adjusted width for mobile */}
              </h2>
              <p className="mt-6 text-base text-gray-600 max-w-2xl mx-auto">Ce que disent nos clients Revoobit satisfaits</p> {/* Adjusted mt and text size for mobile */}
            </div>
      ...
      <section className={cn("py-16 text-white text-center px-4", secondaryColorClass)} style={{ background: `linear-gradient(135deg, var(--${siteData.secondaryColor}-700) 0%, var(--${siteData.primaryColor}-600) 100%)` }}> {/* Adjusted padding for mobile */}
        <div className="container mx-auto max-w-4xl"> {/* Removed px-4 md:px-6, using container mx-auto */}
          <h2 className="text-3xl font-bold mb-4">Restez Informé sur Revoobit</h2> {/* Adjusted text size for mobile */}
          <p className="text-base md:text-xl mb-8 opacity-90">Inscrivez-vous à notre newsletter pour recevoir nos nouveautés et offres exclusives Revoobit</p> {/* Adjusted text size for mobile */}
